...
import {
  ...
  updateMediaSettingsRequestSchema,
...
const MEDIA_SETTINGS_BODY_LIMIT = 16 * 1024;

export type MediaSettingsRouteDependencies = {
  resolveSession?: ResolveUserSession;
  readSettings?: typeof readMediaSettings;
  updateSettings?: typeof updateMediaSettings;
  now?: () => Date;
  createRevision?: () => string;
};

export function createMediaSettingsRoutes(
  dependencies: MediaSettingsRouteDependencies = {},
) {
  const routes = new Hono<StudioHonoEnvironment>();
  const readSettings = dependencies.readSettings ?? readMediaSettings;
  const updateSettings = dependencies.updateSettings ?? updateMediaSettings;
  ...
  routes.get('/', async (c) => {
    ...
    return c.json(mediaSettingsSuccessSchema.parse({
      success: true,
      data: document,
    }));
  });
  ...
  routes.put('/', bodyLimit({
    maxSize: MEDIA_SETTINGS_BODY_LIMIT,
    ...
    const parsed = updateMediaSettingsRequestSchema.safeParse(body.value);
    ...
    beginAudit(c, { action: 'settings_update', target: { type: 'settings', id: 'media' } });
    ...
    auditSettings(c, 'media', Object.keys(parsed.data.settings));
    return c.json(mediaSettingsSuccessSchema.parse({
      success: true,
