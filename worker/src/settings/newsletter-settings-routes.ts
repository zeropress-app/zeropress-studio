...
const NEWSLETTER_SETTINGS_BODY_LIMIT = 32 * 1024;

export type NewsletterSettingsRouteDependencies = {
  resolveSession?: ResolveUserSession;
  readSettings?: typeof readNewsletterSettings;
  updateSettings?: typeof updateNewsletterSettings;
  now?: () => Date;
  createRevision?: () => string;
};

export function createNewsletterSettingsRoutes(
  dependencies: NewsletterSettingsRouteDependencies = {},
) {
  const routes = new Hono<StudioHonoEnvironment>();
  const readSettings = dependencies.readSettings ?? readNewsletterSettings;
  const updateSettings = dependencies.updateSettings ?? updateNewsletterSettings;
  ...
  routes.get('/', async (c) => {
    ...
    return c.json(newsletterSettingsSuccessSchema.parse({
      success: true,
  ...
  routes.put('/', bodyLimit({
    maxSize: NEWSLETTER_SETTINGS_BODY_LIMIT,
    ...
    const parsed = updateNewsletterSettingsRequestSchema.safeParse(body.value);
    ...
    beginAudit(c, { action: 'settings_update', target: { type: 'settings', id: 'newsletter' } });
    ...
    auditSettings(c, 'newsletter', Object.keys(parsed.data.settings));
    return c.json(newsletterSettingsSuccessSchema.parse({
      success: true,
