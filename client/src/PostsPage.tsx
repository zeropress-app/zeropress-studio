...
import { authorIdSchema } from '../../contracts/authors';
...
import { STUDIO_PATHS, studioPostPath } from './routing/studio-routes';
...
const STATUS_TONE: Record<PostStatus, StatusTone> = {
  published: 'positive',
  draft: 'neutral',
  trash: 'critical',
};

export function PostsPage(input: {
  data: AccountSession;
  onSessionEnded: () => void;
}) {
  ...
  const parsedAuthorFilter = authorIdSchema.safeParse(
    searchParams.get('author_id'),
  );
  const authorFilter = canManageAllPosts && parsedAuthorFilter.success
    ? parsedAuthorFilter.data
    : '';
  ...
  useEffect(() => {
    ...
    void requestPosts({
      ...
      page,
      per_page: POSTS_DEFAULT_PAGE_SIZE,
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    let active = true;
    void requestPostEditorOptions('author', '', controller.signal)
      .then((response) => {
        if (!active) return;
        if (!response.success) {
          if (response.error.code === 'AUTHENTICATION_REQUIRED') {
        ...
        setAuthorOptions(response.data.items);
      }).catch(() => undefined);
    return () => {
      active = false;
      controller.abort();
    };
  }, [canManageAllPosts, input.onSessionEnded]);
  ...
  function failureMessage(current: Failure): string {
    ...
    if (current.code === 'POST_REVISION_CONFLICT') {
  ...
  const filtered = search !== '' || status !== 'all' || authorFilter !== '';
  ...
  return (
    <>
      ...
      >
        ...
        {loadState.access.scope === 'own' ? (
          <Notice tone="info">
            {t('list.ownScope', {
              author: loadState.access.author.display_name,
            })}
          </Notice>
        ) : loadState.access.scope === 'unavailable' ? (
          <Notice tone="warning" title={t('list.authorLinkRequiredTitle')}>
            {t('list.authorLinkRequiredDescription')}
          </Notice>
        ) : null}
        ...
        <div className="content-list-toolbar content-list-toolbar-editorial">
          <div className="content-list-toolbar-controls">
            ...
            {loadState.access.scope === 'all' ? (
              <div className="content-list-author-filter">
                <Field label={t('list.authorFilter')} labelHidden>
                  {(control) => (
                    <select
                      {...control}
                      value={authorFilter}
                    ...
                    >
                      <option value="">{t('list.allAuthors')}</option>
                      {authorFilter && !authorOptions.some((author) => (
            ...
            {loadState.access.scope !== 'unavailable' ? (
              <div className="content-list-create-action">
                <ButtonLink variant="primary" to={STUDIO_PATHS.newPost}>
        ...
        {loadState.items.length === 0 ? (
          <EmptyState
            announce={filtered}
            headingLevel={2}
            title={t(loadState.access.scope === 'unavailable'
              ? 'list.authorLinkRequiredEmptyTitle'
              : filtered ? 'list.emptyFilteredTitle' : 'list.emptyTitle')}
            description={t(loadState.access.scope === 'unavailable'
              ? 'list.authorLinkRequiredEmptyDescription'
              : filtered
          ...
          <>
            ...
            <thead>
              <tr>
                ...
                <th scope="col">{t('list.tableAuthor')}</th>
            ...
            <tbody>
              {loadState.items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td data-label={t('list.tableAuthor')}>
